...
const formateadorMoneda = new Intl.NumberFormat("es-PE", { style: "currency", currency: "PEN" });
...
function actualizarCategorias() {
    ...
    filtroCategoria.value = obtenerCategorias(inventario).includes(seleccionActual) ? seleccionActual : "";
}
...
function renderizar() {
    ...
    document.querySelector("#resumenVisible").textContent = `${visibles.length} producto${visibles.length === 1 ? "" : "s"}`;
    actualizarResumen();
}
