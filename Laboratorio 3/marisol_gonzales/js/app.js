...
function mostrarPerfil(perfil) {
    ...
    ////////
    // reto de extensión 1
    const salidaBiografia = document.querySelector("#salidaBiografia");

    salidaBiografia.textContent =
        perfil.biografia || "Sin biografía registrada.";

    const puntos = [...perfil.biografia].length;

    if (puntos > 200) {
        salidaBiografia.classList.add("biografia-larga");
    } else {
        salidaBiografia.classList.remove("biografia-larga");
    }
    ...
    document.querySelector("#inicialesNombre").textContent =
        `Iniciales: ${iniciales}`;
